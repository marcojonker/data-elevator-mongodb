...
module.exports = {
  ...
  onUp : function(floorWorkerParameters, callback) {
    MongoClientHelpers.getDocuments(floorWorkerParameters, 'dataobjects', function(_error, _database, collection, documents) {
      async.each(documents, 
        function(document, callback) {
          if(document.objectStatus == 1) {
            document.objectStatus = 2;
            collection.update({"_id": document._id}, document, null, function(error, _document) {
